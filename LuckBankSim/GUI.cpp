...
void GUI::loadText(string inText, float X, float Y) {
    sf::Font font;
    font.loadFromFile(ARIAL_FONT);
    textCity.setFont(font);
    textCity.setString(inText);
    textCity.setCharacterSize(15);
    textCity.setFillColor(outlineColor);
    textCity.setStyle(sf::Text::Regular);
    textCity.setPosition(X, Y);
    window.draw(textCity, sf::RenderStates());
}
void GUI::drawCities() {
    City C; int x = 80;
    for (int i = 0; i < 11; i++) {
        loadText(C.Name[i], x, 90);
        x += 80;
    }
}
void GUI::drawBankNotes() {
